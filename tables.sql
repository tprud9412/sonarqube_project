...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` FUNCTION `uf_datetime2string`(dt_ Datetime) RETURNS varchar(10) CHARSET utf8mb3
    NO SQL
    DETERMINISTIC
BEGIN
	DECLARE ti INTEGER ;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `makeCalendar`()
BEGIN
	DECLARE sdate date DEFAULT '2013-01-01';
