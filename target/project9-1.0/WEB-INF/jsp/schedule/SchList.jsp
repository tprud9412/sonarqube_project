...
<head>
    ...
    <script>
        ...
        function changeMonth(delta) {
            var currentYear = parseInt('<c:out value="${searchVO.year}"/>');
            var currentMonth = parseInt('<c:out value="${searchVO.month}"/>');

            // 새로운 월 계산
            var newMonth = currentMonth + delta;

            // 월이 1부터 12 사이에 유지되도록 조정
            if (newMonth > 12) {
                newMonth = 1;
                currentYear++;
            } else if (newMonth < 1) {
            ...
            // URL로 이동
            fn_moveToURL('schList?year=' + currentYear + '&month=' + newMonth, '');
        }
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        ...
        <div class="row">
            ...
            <div class="col-lg-2">
                <button class="btn btn-outline btn-primary" style="margin-top:20px;"
                        onclick="fn_moveToURL('schForm', '')">일정추가
                </button>
            </div>
        </div>
            ...
            <div class="calendarColumnBox">
                ...
                <c:forEach var="items" items="${listview.list}" varStatus="status">
                    <div class="calendarDay"
                         onmouseover="calendarDayMouseover(event, '<c:out value="${items.ssno}"/>', '<c:out
                                 value="${listview.cddate}"/>')" onmouseout="calendarDayMouseout()">
                        <c:if test='${items.userno==sessionScope.userno}'>
                            <a href="schForm?ssno=<c:out value="${items.ssno}"/>&sdseq=<c:out value="${items.sdseq}"/>"><c:out
                                    value="${items.sstitle}"/></a>
                        </c:if>
                        <c:if test='${items.ssno!=null and items.userno!=sessionScope.userno}'>
                            <a href="schRead?ssno=<c:out value="${items.ssno}"/>&sdseq=<c:out value="${items.sdseq}"/>"><c:out
                                    value="${items.sstitle}"/></a>
                        </c:if>
                        <c:if test='${items.ssno==null}'>
                            <span style="color:<c:out value="${items.fontcolor}"/>"><c:out
                                    value="${items.sstitle}"/></span>
                        </c:if>
                    </div>
                </c:forEach>
            </div>
